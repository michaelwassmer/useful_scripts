...
for line in original_config_as_list:
    if "inputDataset" in line:
        original_input_dataset = line.split("=")[1].strip().replace('"','').replace("'","")
    if "requestName" in line:
        request_name = line.split("=")[1].strip().replace('"','').replace("'","")
    if "unitsPerJob" in line:
        units_per_job = line.split("=")[1].strip().replace('"','').replace("'","")
    if "maxMemoryMB" in line:
        max_memory_mb = line.split("=")[1].strip().replace('"','').replace("'","")
...
for line in original_config_as_list:
    ...
    if "requestName" in line or "workArea" in line:
        ...
        modified_config_as_list.append(line.replace(units_per_job,"10000"))
...
# if maxmemory options was not used in original config, add it here
if not max_memory_mb:
    modified_config_as_list.append("config.JobType.maxMemoryMB = 2000")
