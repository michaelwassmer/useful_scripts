...
era_label.SetTextSize(0.05)

probe_triggers_label = ROOT.TLatex(20,1.04,probe_triggers)
probe_triggers_label.SetTextSize(0.04)
...
for mc_eff_set,data_eff_set,mc_label,data_label in zip(mc_effs,data_effs,mc_labels,data_labels):
    c=ROOT.TCanvas()
    l=ROOT.TLegend(0.6,0.33,0.9,0.5)
    line = ROOT.TLine(0,0.95,500,0.95)
    line.SetLineStyle(ROOT.kDashed)
    dummy_eff = mc_eff_set[0].Clone("dummy")
    dummy_eff.SetLineColorAlpha(0,0)
    dummy_eff.SetTitle(";METNoMu (GeV);efficiency")
    dummy_eff.Draw()
    ROOT.gPad.Update()
    dummy_eff.GetPaintedGraph().GetXaxis().SetRangeUser(0,500)
    dummy_eff.GetPaintedGraph().GetXaxis().SetTitleSize(0.05)
    dummy_eff.GetPaintedGraph().GetXaxis().SetLabelSize(0.045)
    dummy_eff.GetPaintedGraph().GetYaxis().SetTitleSize(0.05)
    dummy_eff.GetPaintedGraph().GetYaxis().SetLabelSize(0.045)
    dummy_eff.GetPaintedGraph().GetYaxis().SetTitleOffset(0.9)
    for i,mc_eff in enumerate(mc_eff_set):
        ...
        mc_eff.Draw("same")
        ROOT.gPad.Update()
        l.AddEntry(mc_eff,"{}, {}".format(mc_eff.GetTitle(),mc_eff.GetPaintedGraph().GetXaxis().GetTitle().replace("METNoMu (GeV)","")),"lp")
    for i,data_eff in enumerate(data_eff_set):
        ...
        data_eff.Draw("same")
        ROOT.gPad.Update()
        l.AddEntry(data_eff,"{}, {}".format(data_eff.GetTitle(),data_eff.GetPaintedGraph().GetXaxis().GetTitle().replace("METNoMu (GeV)","")),"lp")
    l.Draw("same")
    line.Draw("same")
    cms_label.Draw("same")
    wip_label.Draw("same")
...
for mc_eff_set,data_eff_set,var_label in zip(mc_effs_t,data_effs_t,variables_labels):
    c=ROOT.TCanvas()
    l=ROOT.TLegend(0.6,0.25,0.9,0.5)
    line = ROOT.TLine(0,0.95,500,0.95)
    line.SetLineStyle(ROOT.kDashed)
    dummy_eff = mc_eff_set[0].Clone("dummy")
    dummy_eff.SetLineColorAlpha(0,0)
    ...
    for i,mc_eff in enumerate(mc_eff_set):
        ...
        mc_eff.Draw("same")
        ROOT.gPad.Update()
        l.AddEntry(mc_eff,"{}".format(mc_eff.GetTitle()),"lp")
    for i,data_eff in enumerate(data_eff_set):
        data_eff.SetLineColor(ROOT.kBlue+1*i*(i%2==0)-1*i*(i%2==1))
        data_eff.SetMarkerStyle(20+1*i)
        data_eff.SetMarkerColor(ROOT.kBlue+1*i*(i%2==0)-1*i*(i%2==1))
        data_eff.Draw("same")
        ROOT.gPad.Update()
        l.AddEntry(data_eff,"{}".format(data_eff.GetTitle()),"lp")
    l.Draw("same")
    line.Draw("same")
    cms_label.Draw("same")
    wip_label.Draw("same")
